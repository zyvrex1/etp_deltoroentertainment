...
const EditEventModal = ({ isOpen, onClose, event }) => {
  ...
  const handleImageDrop = (e) => {
    ...
    if (e.dataTransfer.files[0]) {
      ...
      setFormData({ ...formData, imageFile: file, imagePreviewUrl: URL.createObjectURL(file) });
    }
  };
...
const handleImageRemove = () => {
  ...
  setFormData({
    ...formData,
    booths: [...formData.booths, { size: "", price: 0, quantity: 1 }]
  });
};
...
const removeBooth = (index) => {
  ...
  useEffect(() => {
  if (event) {
    const formatDate = (isoDate) => isoDate ? new Date(isoDate).toISOString().split('T')[0] : today;
...
const handleSaveChanges = async (e) => {
  ...
  if (eventType === "Seating Arrangement") {
    if (!seatMap) empty.push("seatMap");
    if (!seatVariations || seatVariations.length === 0) empty.push("seatVariations");

    seatVariations?.forEach((s, i) => {
      if (!s.seatNumber?.trim() || Number(s.price) < 0 || Number(s.quantity) < 1) {
  ...
  // Booth validation
  booths?.forEach((b, i) => {
    if (!b.size?.trim() || Number(b.price) < 0 || Number(b.quantity) < 1) empty.push(`booths[${i}]`);
  });
  ...
  const totalTicketsToSend =
    totalTickets
      ? Number(totalTickets)
      : seatVariations?.reduce((sum, s) => sum + (Number(s.quantity) || 1), 0) || 0;
  ...
  <div className="general-event-modal-container">
    ...
    <form className="add-event-modal-body add-event-form" onSubmit={handleSaveChanges}>
  ...
  <div className="file-preview">
    ...
    <p className="file-name">
      {formData.imageFile ? formData.imageFile.name : event.image?.split("/").pop()}
    </p>
  ...
  <div className="add-event-form-group">
    <input
      ...
      onChange={(e) =>
        setFormData({ ...formData, venue: { ...formData.venue, name: e.target.value } })
      }
  ...
  <div className="add-event-form-group">
    <input
      ...
      onChange={(e) =>
        setFormData({ ...formData, venue: { ...formData.venue, address: e.target.value } })
      }
  ...
  <div className="add-event-form-row">
    <div className="add-event-form-group">
      <input
        ...
        onChange={(e) =>
          setFormData({ ...formData, venue: { ...formData.venue, city: e.target.value } })
        }
    ...
    <div className="add-event-form-group">
      <input
        ...
        onChange={(e) =>
          setFormData({ ...formData, venue: { ...formData.venue, zipCode: e.target.value } })
        }
      ...
      <div className="add-event-form-group">
        ...
        <div style={{ display: "flex", gap: "20px", marginTop: "5px" }}>
          <label style={{ display: "flex", alignItems: "center", gap: "5px", cursor: "pointer", color: "black" }}>
            <input
              type="radio"
              name="eventType"
              value="General Admission"
              checked={formData.eventType === "General Admission"}
              onChange={() => setFormData({ ...formData, eventType: "General Admission" })}
            />
            General Admission
          </label>
          <label style={{ display: "flex", alignItems: "center", gap: "5px", cursor: "pointer", color: "black" }}>
            <input
              ...
              onChange={() => setFormData({ ...formData, eventType: "Seating Arrangement" })}
            />
            Assigned Seating
          </label>
        </div>
        ...
        <div className="add-event-form-row">
          <div className="add-event-form-group">
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, ticketPrice: e.target.value === "" ? "" : Number(e.target.value) })
              }
            />
          </div>
          <div className="add-event-form-group">
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, totalTickets: e.target.value === "" ? "" : Number(e.target.value) })
              }
            />
          </div>
        </div>
