...
export default function ReportsAnalytics() {
    ...
    useEffect(() => {
        const fetchAnalytics = async () => {
            ...
            try {
                ...
                const [topData, overviewData, allReservations] = await Promise.all([
                    analyticsService.getTopPerformingEvents(user.token, dateRange.start, dateRange.end, 'all'),
                    analyticsService.getOverviewStats(user.token, dateRange.start, dateRange.end, selectedEventId),
                ...
                if (overviewData) {
                    ...
                    if (selectedEventId !== 'all') {
                        // For a specific event, use ALL its reservations (lifetime)
                        inRange = (allReservations || []).filter(r =>
                            r.event && (r.event === selectedEventId || r.event._id === selectedEventId || r.event.id === selectedEventId)
                        );
                        // No previous period comparison for event-level view
                    } else {
                        ...
                        prevInRange = (allReservations || []).filter(r => {
                            const d = new Date(r.createdAt);
        ...
        fetchAnalytics();
    }, [user?.token, dateRange, selectedEventId]);
    ...
    // Category and Revenue Trends are now dynamic.
    ...
    const selectedEvent = allEventsList.find(e => (e._id || e.id) === selectedEventId);
    let seatTerm = "Seats";
    let seatLabel = "Seats Sold";
    if (selectedEventId === "all") {
        seatTerm = "Seats / Tickets";
        seatLabel = "Seats / Tickets Sold";
    } else if (selectedEvent?.eventType === 'General Admission') {
        seatTerm = "Tickets";
        seatLabel = "Tickets Sold";
    } else {
        seatTerm = "Seats";
        seatLabel = "Seats Sold";
    }

    const categoryData = [
        { name: "Booth", value: overviewStats.boothsSold || 0, color: "#0059ff" },
        { name: seatTerm, value: overviewStats.ticketsSold || 0, color: "#8c52ff" },
    ...
    const exportReport = async () => {
        ...
        try {
            ...
            const rangeLabel = dateRange.preset === 'all'
                ? "Period: All Time"
                : `Period: ${dateRange.start.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })} – ${dateRange.end.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}`;
            ...
            const categories = [
                ...
                { label: `${seatTerm} Sales`, value: overviewStats.ticketsSold || 0, color: [140, 82, 255] },
            ...
            const occupancyItems = [
                ...
                { label: `${seatTerm} Occupancy`, pct: overviewStats.seatOccupancy || 0, color: [22, 163, 74] },
            ];
            ...
            const eventHeaders = ['#', 'Event Name', `${seatTerm} / Booths`, 'Revenue', 'Capacity'];
            const eventRows = topEvents.map((e, i) => {
                const eventSeatTerm = e.eventType === 'General Admission' ? 'tickets' : 'seats';
                return [
                    `#${i + 1}`,
                    e.name,
                    `${e.ticketsSold} ${eventSeatTerm} • ${e.boothsSold} booths`,
                    e.revenueText,
                    e.cap,
                ];
            });
            ...
            <div className="stats-grid-wrapper">
                <div className="stats-grid">
                    ...
                    <div className="report-stat-card">
                        ...
                        <div className="bottom-stats">
                            <p className="regular-body-text left-aligned">{seatLabel}</p>
                                                ...
                                                <div className="event-name-cell">
                                                    ...
                                                    <div className="event-info-wrapper left-aligned">
                                                        ...
                                                        <span className="event-sub-text smaller-body-text">
                                                            {row.ticketsSold} {row.eventType === 'General Admission' ? 'tickets' : 'seats'} sold • {row.boothsSold} booths sold
                                                        </span>
                                                    </div>
                                                </div>
                                            </td>
                ...
                <div className="reports-right-panel">
                    <div className="chart-card donut-chart-card">
                        ...
                        <div className="chart-placeholder donut">
                            ...
                            <div className="chart-legend multi">
                                ...
                                <span className="legend-item"><span className="dot purple"></span>{seatTerm} ({getPercent(categoryData[1].value)}%)</span>
                    ...
                    <div className="list-card sponsor-metrics-card">
                        ...
                        <div className="progress-list">
                            ...
                            <div className="progress-group">
                                <div className="progress-header">
                                    <span className="small-body-text label-text">{seatTerm} Occupancy</span>
