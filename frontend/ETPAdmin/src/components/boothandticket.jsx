...
const BoothandTicket = () => {
  ...
  // Booth Map: 6x5 grid. Each cell: null (empty) or { code, type, status, dimensions, bookedBy? }
  const boothGrid = [
    [{ code: "V1", type: "vip", status: "booked", dimensions: "20x20", bookedBy: "Acme Corp" }, { code: "V2", type: "vip", status: "available", dimensions: "20x20" }, null, null, null, null],
    [null, { code: "C1", type: "corner", status: "available", dimensions: "10x10" }, { code: "I1", type: "inline", status: "available", dimensions: "10x10" }, null, null, null],
    [null, null, null, { code: "I2", type: "inline", status: "booked", dimensions: "10x10", bookedBy: "TechStart Inc" }, null, null],
                  ...
                  {boothGrid.map((row, ri) =>
                    row.map((cell, ci) => (
                      <button
                        type="button"
                        key={`${ri}-${ci}`}
                        className={`bt-booth-cell ${cell ? `filled status-${cell.status}` : "empty"}`}
                      ...
                      >
                        {cell ? (
                          <>
                            <span className="bt-booth-code">{cell.code}</span>
                            <span className="bt-booth-type">({cell.type})</span>
                          </>
                        ) : null}
                      </button>
                    ))
